...
if ($_POST) {
    ...
    $result = mysqli_query($con, "select * from admin where admin_mail='{$txtAdminEmail}'") or die(mysqli_error($con));
...
// Instantiation and passing `true` enables exceptions
        ...
        try {
            ...
            $mail->addAddress($txtAdminEmail, $row["admin_name"]);     // Add a recipient
            // Content
            $mail->isHTML(true);                                  // Set email format to HTML
            $mail->Subject = 'Recovery password';
            $mail->Body = 'password : ' . $row["admin_password"];
            $mail->AltBody = 'password:' . $row["admin_password"];
            ;
                                ...
                                <div class="form-group">
                                    <div class="row">
                                        <div class="col-xs-6 text-xs-right forgot_pwd">
                                            <a href="admin-login.php" class="custom-control-description forgottxt_clr">Click to Login</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
