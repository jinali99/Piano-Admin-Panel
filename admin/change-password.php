...
if ($_POST) {
    ...
    $select = mysqli_query($con, "select * from admin where `admin_id`='{$admin_id}'")or die(mysqli_error($con));
    $result = mysqli_fetch_assoc($select);
    if ($result["admin_password"] == $oldpassword) {
        if ($newpassword == $confirmpassword) {
            $update = mysqli_query($con, "UPDATE admin SET `admin_password`='{$confirmpassword}' where `admin_id`='{$admin_id}'")or die(mysqli_error($con));
            if ($update) {
